...
namespace MarkEmbling.PostcodesIO.Tests.Unit
{
    ...
    public class RequestExecutorTests
    {
        ...
        [Test]
        public void ExecuteRequest_returns_empty_collection_instead_of_null()
        {
            var clientMock = new Mock<IRestClient>();
        ...
        [Test]
        public void ExecuteRequest_returns_null_if_its_not_a_collection()
        {
            var clientMock = new Mock<IRestClient>();
            var responseMock = new Mock<IRestResponse<RawResult<object>>>();
            var rawResultMock = new Mock<RawResult<object>>();
        ...
        [Test]
        public async Task ExecuteRequestAsync_returns_the_result_when_there_is_one()
        {
            var resultData = new object();
            ...
            var sut = new RequestExecutor(clientMock.Object);
            var result = await sut.ExecuteRequestAsync<object>(new RestRequest());

            Assert.AreSame(resultData, result);
        }

        [Test]
        public void ExecuteRequestAsync_throws_exception_when_response_is_null()
        {
            var resultData = new object();
            ...
            var sut = new RequestExecutor(clientMock.Object);
            Assert.ThrowsAsync<PostcodesIOEmptyResponseException>(
                async () => await sut.ExecuteRequestAsync<object>(new RestRequest()));
        }

        [Test]
        public void ExecuteRequestAsync_throws_when_request_results_in_error()
        {
            var expectedInnerException = new Exception();
            ...
            var sut = new RequestExecutor(clientMock.Object);
            var resultException = Assert.ThrowsAsync<PostcodesIOApiException>(
                async () => await sut.ExecuteRequestAsync<object>(new RestRequest()));
            Assert.AreSame(expectedInnerException, resultException.InnerException);
        }

        [Test]
        public async Task ExecuteRequestAsync_returns_empty_collection_instead_of_null()
        {
            var clientMock = new Mock<IRestClient>();
            var responseMock = new Mock<IRestResponse<RawResult<List<object>>>>();
            var rawResultMock = new Mock<RawResult<List<object>>>();

            clientMock.Setup(x => x.ExecuteTaskAsync<RawResult<List<object>>>(It.IsAny<IRestRequest>()))
                .Returns(Task.FromResult(responseMock.Object));
            responseMock.SetupGet(x => x.Data).Returns(rawResultMock.Object);
            rawResultMock.SetupGet(x => x.Result).Returns((List<object>)null);

            var sut = new RequestExecutor(clientMock.Object);
            var result = await sut.ExecuteRequestAsync<List<object>>(new RestRequest());

            Assert.AreEqual(new List<object>(), result);
        }

        [Test]
        public async Task ExecuteRequestAsync_returns_null_if_its_not_a_collection()
        {
            var clientMock = new Mock<IRestClient>();
            var responseMock = new Mock<IRestResponse<RawResult<object>>>();
            var rawResultMock = new Mock<RawResult<object>>();
